...
/**
 * Enforces admin appearance settings by outputting custom CSS.
 */
final class AppearanceEnforcer {
	...
	public function output_css(): void {
		...
		// Sidebar background.
		$sidebar_bg = $settings[ Constants::APPEAR_SIDEBAR_BG ] ?? '';
		if ( '' !== $sidebar_bg ) {
			$css[] = sprintf( '#adminmenuback, #adminmenu, #adminmenuwrap { background-color: %s !important; }', esc_attr( $sidebar_bg ) );
		}

		// Sidebar text color.
		$sidebar_text = $settings[ Constants::APPEAR_SIDEBAR_TEXT ] ?? '';
		if ( '' !== $sidebar_text ) {
		...
		$sidebar_hover = $settings[ Constants::APPEAR_SIDEBAR_HOVER ] ?? '';
		if ( '' !== $sidebar_hover ) {
		...
		$sidebar_text_hover = $settings[ Constants::APPEAR_SIDEBAR_TEXT_HOVER ] ?? '';
		...
		// Sidebar font size.
		$sidebar_font_size = $settings[ Constants::APPEAR_SIDEBAR_FONT_SIZE ] ?? '';
		...
		// Sidebar width.
		$sidebar_width = $settings[ Constants::APPEAR_SIDEBAR_WIDTH ] ?? '';
		...
		$sidebar_align = $settings[ Constants::APPEAR_SIDEBAR_ALIGN ] ?? '';
		...
		$admin_bg = $settings[ Constants::APPEAR_ADMIN_BG ] ?? '';
		if ( '' !== $admin_bg ) {
		...
		$bg_image_id = $settings[ Constants::APPEAR_BODY_BG_IMAGE ] ?? 0;
		if ( $bg_image_id ) {
			$bg_url = wp_get_attachment_image_url( (int) $bg_image_id, 'full' );
			if ( $bg_url ) {
				$bg_size = $settings[ Constants::APPEAR_BODY_BG_SIZE ] ?? 'cover';
				$css[] = sprintf(
		...
		// Admin text color.
		$text_color = $settings[ Constants::APPEAR_ADMIN_TEXT_COLOR ] ?? '';
		if ( '' !== $text_color ) {
			$css[] = sprintf( '#wpwrap, #wpwrap p, #wpwrap label, #wpwrap th, #wpwrap td { color: %s !important; }', esc_attr( $text_color ) );
		}

		// Admin link color.
		$link_color = $settings[ Constants::APPEAR_ADMIN_LINK_COLOR ] ?? '';
		if ( '' !== $link_color ) {
			$css[] = sprintf( '#wpwrap a { color: %s !important; }', esc_attr( $link_color ) );
		}

		// Button color.
		$btn_color = $settings[ Constants::APPEAR_ADMIN_BTN_COLOR ] ?? '';
		if ( '' !== $btn_color ) {
		...
		// Button text color.
		$btn_text = $settings[ Constants::APPEAR_ADMIN_BTN_TEXT ] ?? '';
		if ( '' !== $btn_text ) {
			$css[] = sprintf( '.wp-core-ui .button-primary, .wp-core-ui .button-primary { color: %s !important; }', esc_attr( $btn_text ) );
		}

		// Border color.
		$border_color = $settings[ Constants::APPEAR_ADMIN_BORDER_COLOR ] ?? '';
		if ( '' !== $border_color ) {
